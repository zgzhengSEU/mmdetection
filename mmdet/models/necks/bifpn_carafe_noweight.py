...
class BiFPNStage(nn.Module):
    ...
    def forward(self, x):
        ...
        p6_up = self.conv6_up(
            self.combine(p6_in + self.p6_upsample_CARAFE(p7_in)))
        ...
        p5_up = self.conv5_up(
            self.combine(p5_in + self.p5_upsample_CARAFE(p6_up)))
        ...
        p4_up = self.conv4_up(
            self.combine(p4_in + self.p4_upsample_CARAFE(p5_up)))
        ...
        p3_out = self.conv3_up(
            self.combine(p3_in + self.p3_upsample_CARAFE(p4_up)))
        ...
        p4_out = self.conv4_down(
            self.combine(p4_in + p4_up + self.p4_down_sample(p3_out)))
        ...
        p5_out = self.conv5_down(
            self.combine(p5_in + p5_up + self.p5_down_sample(p4_out)))
        ...
        p6_out = self.conv6_down(
            self.combine(p6_in + p6_up + self.p6_down_sample(p5_out)))
        ...
        p7_out = self.conv7_down(
            self.combine(p7_in + self.p7_down_sample(p6_out)))
        return p3_out, p4_out, p5_out, p6_out, p7_out

class reBiFPNStage(nn.Module):
    '''
        in_channels: List[int], input dim for P3, P4, P5
        out_channels: int, output dim for P2 - P7
        first_time: int, whether is the first bifpnstage
