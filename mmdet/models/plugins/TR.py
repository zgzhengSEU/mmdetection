    ...
    def __init__(self,
        ...
        if self.q_stride > 1:
            self.q_downsample = nn.AvgPool2d(
                kernel_size=2, stride=self.q_stride)
        else:
            self.q_downsample = None

        if self.kv_stride > 1:
            self.kv_downsample = nn.AvgPool2d(
                kernel_size=2, stride=self.kv_stride)
        else:
...
if __name__ == '__main__':
    ...
    imgs = torch.randn(8, 128, 80, 40)
    model = ChannelTR(in_channels=128, num_heads=8)
    out = model(imgs)
    print(out.shape)
    assert out.shape == imgs.shape
